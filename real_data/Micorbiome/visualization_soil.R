...
CAR <- get_CAR_MB(soil_cgdperes$B, soil_cgdperes$Omega)

among_spp <- graph_from_adjacency_matrix(CAR$C,mode = "directed",weighted = T,diag = F)
abs_graph <- graph_from_adjacency_matrix(abs(soil_cgdperes$Omega),mode = "undirected",weighted = T,diag = F)
...
vertices_df <- data.frame(id=c(paste0("M",1:(ncol(soil_Y))),paste0("E",1:ncol(soil_X))),group = c(rep("microbe",(ncol(soil_Y))),rep("env",ncol(soil_X))))

ind_mat_micro <- expand.grid(from = 1:(ncol(soil_Y)),to = 1:(ncol(soil_Y)))
...
ind_mat_env <- expand.grid(from = 1:ncol(soil_X),to = 1:(ncol(soil_Y)))
...
V(full_graph)$name <- c(colnames(soil_Y),"fertilizer","crop agriculture","poorly drained","total N","October measure")

V(full_graph)$alpha_centrality <- alpha_centrality(full_graph)
V(full_graph)$type <- type[c(rep(2,(ncol(soil_Y))),rep(1,ncol(soil_X)))]
...
ggsave("./real_data/Micorbiome/soil2.pdf",p2,width = 8,height = 7)
