                  ...
                  int verbose)
{
    ...
    // re-scale the re-centered columns of X to have norm sqrt(n-1), (only n-1 samples has regression )
    ...
    for (int j = 0; j < p; j++)
    {
        ...
        X.col(j) -= tmp_mu_x;
        tmp_weight_x = norm(X.col(j)) / sqrt(n-1);
