//
//  GCSSL.cpp
//  Source code for Gaussian Chain Graphical Spike-and-Slab LASSO (GCGSSL)
...
//  Created by Yunyi Shen and partially due to Sameer Deshpande in August 2021.
//
               ...
               int verbose)
{
  ...
  */
  ...
  arma::mat tYY = Y.t() * Y;
  arma::mat tXX = X.t() * X;
  //arma::mat tXY = X.t() * Y; // not used anywhere
  arma::mat mu_mat = X * B;

  arma::mat R = Y * Omega - mu_mat;
  arma::mat tXR = X.t() * R;
  arma::mat tRR = R.t() * R;
  arma::mat S_B = tRR / n; // the copy of S used for updating B
  arma::mat M = mu_mat.t() * mu_mat / n;
  arma::mat S_Omega = tYY / n; // S used for cgQUIC

  arma::vec s_eval = eig_sym(S_B); // vector of eigenvalues of S_B
  ...
  arma::mat S_B_restart = S_B;
  arma::mat S_Omega_restart = S_Omega;
  arma::mat M_restart = M;
  ...
  arma::mat S_B_left = S_B;
  arma::mat S_Omega_left = S_Omega;
  arma::mat M_left = M;
  ...
  arma::mat S_B_down = S_B;
  arma::mat S_Omega_down = S_Omega;
  arma::mat M_down = M;
  ...
  arma::mat S_B_diag = S_B;
  arma::mat S_Omega_diag = S_Omega;
  arma::mat M_diag = M;
  ...
  while (iter < max_iter)
  {
    ...
    B_old = B;
    Omega_old = Omega;
    old_obj = cgobjective(n, p, q, S_Omega, B, X, Omega, Sigma, lambda1, lambda0, xi1, xi0, theta, eta, diag_penalty, theta_hyper_params, eta_hyper_params);
    ...
    update_B_theta(n, p, q, B, R, tXR, S_B, theta, Sigma, eta, X, tXX, lambda1, lambda0, xi1, xi0, diag_penalty, theta_hyper_params, eta_hyper_params, max_iter, eps, verbose);
    ...
    tXR = X.t() * R;
    tRR = R.t() * R;
    S_B = tRR / n; // the copy of S used for updating B
    ...
    obj = cgobjective(n, p, q, S_Omega, B, X, Omega, Sigma, lambda1, lambda0, xi1, xi0, theta, eta, diag_penalty, theta_hyper_params, eta_hyper_params);
    s_eval = eig_sym(S_B);
    if (s_eval(q - 1) / s_eval(0) > s_max_condition)
  ...
  S_B_path.slice(current_index) = S_B;
  S_Omega_path.slice(current_index) = S_Omega;
  M_path.slice(current_index) = M;
  theta_path(current_index) = theta;
  eta_path(current_index) = eta;

  obj_path(current_index) = cgobjective(n, p, q, S_Omega, B, X, Omega, Sigma, lambda1, lambda_spike(L - 1), xi1, xi_spike(L - 1), theta, eta, diag_penalty, theta_hyper_params, eta_hyper_params);
  ...
  for (s = 1; s < L; s++)
  {
    ...
    else
    {
      ...
      S_B = S_B_path.slice(left_index);
      S_Omega = S_Omega_path.slice(left_index);
      M = M_path.slice(left_index);
  ...
  for (t = 1; t < L; t++)
  {
    ...
    else
    {
      ...
      S_B = S_B_path.slice(down_index);
      S_Omega = S_Omega_path.slice(down_index);
      M = M_path.slice(down_index);
  ...
  for (s = 1; s < L; s++)
  {
    for (t = 1; t < L; t++)
    {
      ...
      { // solution to left was unstable. use the re-start
        ...
        S_B_left = S_B_restart;
        S_Omega_left = S_Omega_restart;
        M_left = M_restart;
      ...
      else
      {
        ...
        S_B_left = S_B_path.slice(left_index);
        S_Omega_left = S_Omega_path.slice(left_index);
        M_left = M_path.slice(left_index);
        theta_left = theta_path(left_index);
        eta_left = eta_path(left_index);
      }
      obj_left = cgobjective(n, p, q, S_Omega_left, B_left, X, Omega_left, Sigma_left, lambda1, lambda0, xi1, xi0, theta_left, eta_left, diag_penalty, theta_hyper_params, eta_hyper_params);
      if (early_term(down_index) == 1)
      {
        B_down = B_restart;
        ...
        S_B_down = S_B_restart;
        S_Omega_down = S_Omega_restart;
        M_down = M_restart;
      ...
      else
      {
        ...
        S_B_down = S_B_path.slice(down_index);
        S_Omega_down = S_Omega_path.slice(down_index);
        M_down = M_path.slice(down_index);
        theta_down = theta_path(down_index);
        eta_down = eta_path(down_index);
      }
      obj_down = cgobjective(n, p, q, S_Omega_down, B_down, X, Omega_down, Sigma_down, lambda1, lambda0, xi1, xi0, theta_down, eta_down, diag_penalty, theta_hyper_params, eta_hyper_params);
      if (early_term(diag_index) == 1)
      {
        ...
        S_B_diag = S_B_restart;
        S_Omega_diag = S_Omega_restart;
        M_diag = M_restart;
      ...
      else
      {
        ...
        S_B_diag = S_B_path.slice(diag_index);
        S_Omega_diag = S_Omega_path.slice(diag_index);
        M_diag = M_path.slice(diag_index);
        theta_diag = theta_path(diag_index);
        eta_diag = eta_path(diag_index);
      }
      obj_diag = cgobjective(n, p, q, S_Omega_diag, B_diag, X, Omega_diag, Sigma_diag, lambda1, lambda0, xi1, xi0, theta_diag, eta_diag, diag_penalty, theta_hyper_params, eta_hyper_params);
      ...
      obj_path(current_index) = cgobjective(n, p, q, S_Omega, B, X, Omega, Sigma, lambda1, lambda_spike(L - 1), xi1, xi_spike(L - 1), theta, eta, diag_penalty, theta_hyper_params, eta_hyper_params); // compute at the maximal value
