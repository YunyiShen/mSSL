#ifndef CGLASSO_CGLASSOPATH_H
#define CGLASSO_CGLASSOPATH_H

// this is a naive path following method for cgLASSO with uniform L1 penalty and hot starts, not particularly efficient

#include <RcppArmadillo.h>
#include <linconGaussR.h>
#include <QUIC.h>
#include <workingparam.h>
#include "cgLASSO_B_update.h"
...
List cgLASSO_path(workpara instance,
               arma::vec lambdas,
               arma::vec xis,
               int diag_penalty,
               int max_iter,
               ...
               int nskp = 5)
{
  ...
  */
  ...
  int L = lambdas.n_elem;
  double lambda = 0.0;
  double xi = 0.0;
  arma::mat xi_star(q, q, fill::zeros);
  ...
  // We need copies of B, Omega, R, tXR, S,  and eta that are propogated along the grid
  arma::mat B_left = B;
  ...
  while (iter < max_iter)
  {
    ...
    B_old = B;
    Omega_old = Omega;
    old_obj = cglassoobjective(n, p, q, instance.S, B, instance.X, Omega, Sigma, lambda, xi, diag_penalty);
    ...
    xi_star.fill(xi);
    if (diag_penalty == 1)
      xi_star.diag().fill(xi);
    else if (diag_penalty == 0)
      xi_star.diag().fill(0);

    // M Step Update of B and Theta
    update_B(instance.n_B, p, q, B, instance.R, instance.tXR, instance.S, Sigma, instance.X, instance.tXX, lambda, xi, diag_penalty, max_iter, eps, verbose);
    instance.update_M(B);
    ...
    obj = cglassoobjective(n, p, q, instance.S_Omega, B, instance.X, Omega, Sigma, lambda, xi,  diag_penalty);
  ...
  obj_path(current_index) = cglassoobjective(n, p, q, instance.S, B, instance.X, Omega, Sigma, lambdas(L - 1), xis(L - 1),  diag_penalty);
  ...
  for (s = 1; s < L; s++)
  {
    ...
    while (iter < max_iter)
    {
      ...
      obj = cglassoobjective(n, p, q, instance.S, B, instance.X, Omega, Sigma, lambda, xi,  diag_penalty);
  ...
  for (s = 1; s < L; s++)
  {
    for (t = 1; t < L; t++)
    {
      ...
      obj_left = cglassoobjective(n, p, q, instance_left.S, B_left, instance.X, Omega_left, Sigma_left, lambda, xi, diag_penalty);
      if (early_term(down_index) == 1)
      {
        B_down = B_restart;
      ...
      obj_down = cglassoobjective(n, p, q, instance_down.S, B_down, instance.X, Omega_down, Sigma_down, lambda, xi, diag_penalty);
      ...
      obj_diag = cglassoobjective(n, p, q, instance_diag.S, B_diag, instance.X, Omega_diag, Sigma_diag, lambda, xi, diag_penalty);
      ...
      //obj_path(current_index) = cglassoobjective(n, p, q, S_Omega, B, Omega, Sigma,lambda1, lambdas(L-1), xi, xis(L-1),  diag_penalty);
      obj_path(current_index) = cglassoobjective(n, p, q, instance.S, B, instance.X, Omega, Sigma,  lambdas(L - 1), xis(L - 1),  diag_penalty); // compute at the maximal value
